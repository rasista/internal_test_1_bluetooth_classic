...
void command_parser(uint8_t argc, char **argv)
{
  ...
  while (1) {
    ...
    cmd_lut_entry =
      get_command_entry(&argv[current_cmd_index], current_command_lut);
    ...
    if (cmd_lut_entry != NULL) {
      ...
      if (cmd_lut_entry->handler == NULL) {
        ...
        cmd_lut_entry->handler((argc - max_no_of_args - 1),
                               &argv[current_cmd_index + 1]);
        break;
      }
    } else {
      // call get_module_entry to get the sub_modules from current_module_lut
      module_lut_entry =
        get_module_entry(&argv[current_cmd_index], current_module_lut);
    }
    ...
    if (module_lut_entry != NULL) {
      ...
      if (module_lut_entry->custom_handler == NULL) {
        ...
        if (max_no_of_args < argc) {
          ...
          // check if modules_lut_entry->submodules != NULL then update the
          // current_module_lut and call display fucntion
          if (module_lut_entry->sub_modules != NULL) {
            current_module_lut = module_lut_entry->sub_modules;
          ...
          // check if modules_lut_entry->commands != NULL then update the
          // current_command_lut and call display fucntion
          if (module_lut_entry->commands != NULL) {
            current_command_lut = module_lut_entry->commands;
    ...
    } else { // if user input wrong sub_module and commands then call display
             // fucntion
      printf("\r\n Enter correct module_name from below list with . in between "
             "the args \r\n");
      // call display sub_module function
      display_submodule(&argv[current_cmd_index], current_module_lut);
      printf("\r\n Enter correct command_name from below list with . in "
             "between the args \r\n");
      // call display command function
      display_command(&argv[current_cmd_index], current_command_lut);
      break;
 ...
 * @return pointer to current_command if command_name is not NULL else return
 * NULL
 */
...
void display_submodule(char **argv, module_lut_entry_t *current_module)
{
  ...
  while (current_module[i].module_name != NULL) {
    ...
    strncat(buff, (char *)current_module[i].module_name,
            strlen((char *)current_module[i].module_name));
    strcat(buff, "\n");
    i++;
  }
...
void display_command(char **argv, command_lut_entry_t *current_command)
{
  ...
  while (current_command[j].command_name != NULL) {
    ...
    strncat(buff, (char *)current_command[j].command_name,
            strlen((char *)current_command[j].command_name));
    strcat(buff, "\n");
    j++;
  }
...
void prepare_cmd_args(uint8_t *uart_buff)
{
  int j = 0, i = 0;
  uint8_t num_args = 0;
  // int var = strlen(argv[1]) + 1;
  // delimiter " . "
  const char *delimiter_str = " ";
  char *token; // str[var];
  char *strs_ptr[MAX_NO_OF_LENGTH];
  // argv[1] is the input string throught command line
  // strncpy(str, argv[1], var);
  // strtok is used to break given string into a series of tokens using the
  // delimiter
  token = strtok((char*)uart_buff, delimiter_str);
  while (token != NULL) {
    strs_ptr[i] = malloc((strlen(token) + 1) * sizeof(char));
    if (strs_ptr[i] == NULL) {
      // printf("/* Memory not allocated */\n");
      return;
    } else {
      // copying the token into allocated memory
      strcpy(strs_ptr[i], token);
  ...
  strs_ptr[i] = NULL;
  // calling command_parser func with params as no of args and
  // strs_ptr(pointers to strings)
  command_parser(num_args, strs_ptr);
  for (; strs_ptr[j] != NULL; j++) {
