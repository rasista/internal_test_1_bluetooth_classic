...
def pytest_addoption(parser):
    ...
    parser.addoption('--clean', action='store_true', default=False, help='Make clean')
    parser.addoption('--disable-reset', help='Disable the test environment reset')
...
def pytest_configure(config):
    ...
    global disable_reset_glbl
    disable_reset = config.getoption('disable_reset')
    disable_reset_glbl = 3
    if disable_reset == "post":
        disable_reset_glbl = 2
    elif disable_reset == "pre":
        disable_reset_glbl = 1
    print(f"-------------------------------------------------------{disable_reset_glbl}-------------------------------------------------------")
...
def pytest_runtest_makereport(item, call):
    if call.when == 'call':
        ...
        if test_report:
            ...
            killSetup()
            subprocess.run(["bash", f"{docker_base_path}/tools/run_gcov.sh", "ulc"])
...
def pytest_collection_modifyitems(session, config, items):
    print(items)
    for item in items:
        # Check if the test doesn't already have a config marker
        if not any(marker.name == 'config' for marker in item.iter_markers()):
            # Add the UART configuration marker
            uart_config = pytest.mark.config(
                'duts',
                uart={
                    0: {'port': f'{BUILD_PATH}/D1_UARTpy'},
                    1: {'port': f'{BUILD_PATH}/D2_UARTpy'}
                }
            )
            item.add_marker(uart_config)
