/**
 * @file acl_peripheral.c
 * @brief
 * @version 0.1
 * @date 2023-01-18
 *
 */

#include "acl_peripheral.h"
#include "btc_device.h"
#include "btc_rail.h"
#include "acl_peripheral_sm_autogen.h"
#include "acl_common.h"
#include "lpw_scheduler.h"
#include "arqn_seqn_check.h"
#ifdef SIM
#include "rail_simulation.h"
#else
#include "btc_rail_intf.h"
extern shared_mem_t *ulc_llc_share_mem;
#endif

static void configure_and_schedule_tx_peri(connection_info_t *connection_info_p, uint32_t clk)
{
  uint32_t status;
  sm_t *connection_info_sm_p = connection_info_p->connection_info_sm;
  btc_trx_configs_t *btc_trx_configs = &connection_info_p->trx_config;

  // Handle multi-slot timing: TX scheduling always follows RX, so only check last_rx_slots
  if (connection_info_sm_p->last_rx_slots > 1) {
    uint32_t BtcDate;
    uint64_t ProtimerDate;
    RAIL_SEQ_BTC_GetLastRxDate(btc_trx_configs->bd_clk_counter_handle.bd_clk_counter_id, &BtcDate, &ProtimerDate);
    btc_trx_configs->btc_rail_trx_config.clk = BtcDate + (connection_info_sm_p->last_rx_slots * SLOT_WIDTH);
    connection_info_sm_p->last_rx_slots = 0;
  } else {
    configure_conn_slot_params(connection_info_p, clk, btc_trx_configs);
  }

  packetization_acl(connection_info_p, btc_trx_configs); // TODO: Can be removed. Required for simulation tests
  update_channel_info(btc_trx_configs, &connection_info_p->btc_fh_data);

#ifndef SIM
  linear_buffer_t *linear_buffer_ptr = NULL;
  arqn_seqn_scheme_status_t *arqn_seqn_status = &connection_info_p->acl_arqn_seqn_info;
  if ((linear_buffer_ptr = (linear_buffer_t *)get_tx_from_fifo_if_available()) != NULL  && arqn_seqn_status->remote_flow) {
    uint32_t len = linear_buffer_ptr->length;
    uint8_t *tx_pkt = (uint8_t *)linear_buffer_ptr->data;
    tx_packetization(connection_info_p, btc_trx_configs, tx_pkt, len);
  } else {
#endif
  form_null_arqn_flow(btc_trx_configs, connection_info_p);
  // Set the arqn status to NAK for the next packet to convery that the current packet does not require an ACK
#ifndef SIM
  arqn_seqn_status->arqn_status = NAK;
}
#endif
  update_encryption_parameters(connection_info_p, btc_trx_configs, true, PERIPHERAL_TO_CENTRAL);

  // Set last_tx_slots based on transmitted packet type (applies to both data and POLL/NULL packets)
  connection_info_sm_p->last_tx_slots = packet_type_slot_lookup_table[btc_trx_configs->btc_rail_trx_config.packetConfig.packetType][connection_info_p->br_edr_mode];

  status = schedule_tx(btc_trx_configs);
  SM_STATE(connection_info_sm_p, ACL_PERIPHERAL_TX_SCHEDULED);
  if (status != BTC_RAIL_PASS_STATUS) {
    SM_STEP(ACL_PERIPHERAL_SM, connection_info_sm_p, TX_ABORT);
  }
}

static void configure_and_schedule_rx_peri(connection_info_t *connection_info_p, uint32_t clk)
{
  uint32_t status;
  sm_t *connection_info_sm_p = connection_info_p->connection_info_sm;
  btc_trx_configs_t *btc_trx_configs = &connection_info_p->trx_config;

  // Handle multi-slot timing: RX scheduling always follows TX, so only check last_tx_slots
  if (connection_info_sm_p->last_tx_slots > 1) {
    btc_trx_configs->btc_rail_trx_config.clk = btc_trx_configs->btc_rail_trx_config.clk + connection_info_sm_p->last_tx_slots * SLOT_WIDTH;
    connection_info_sm_p->last_tx_slots = 0;
  } else {
    configure_conn_slot_params(connection_info_p, clk, btc_trx_configs);
  }

  update_channel_info(btc_trx_configs, &connection_info_p->btc_fh_data);
  configure_trx_acl_rx(btc_trx_configs, connection_info_p->br_edr_mode, BTC_RAIL_RX_TIMEOUT);
  update_encryption_parameters(connection_info_p, btc_trx_configs, false, CENTRAL_TO_PERIPHERAL);
  btc_trx_configs->rx_hdr_needed = 1;
  status = schedule_rx(btc_trx_configs);
  SM_STATE(connection_info_sm_p, ACL_PERIPHERAL_WAITING_FOR_RX_PKT_HEADER);
  if (status != BTC_RAIL_PASS_STATUS) {
    SM_STEP(ACL_PERIPHERAL_SM, connection_info_sm_p, RX_ABORT);
  }
}

static void handle_tx_rx_peri(connection_info_t *connection_info_p, bool is_tx, uint32_t clk)
{
  sm_t *connection_info_sm_p = connection_info_p->connection_info_sm;
  btc_trx_configs_t *btc_trx_configs = &connection_info_p->trx_config;
  if (IS_BT_CLOCK_LT(btc_get_current_bd_clk_cnt(&btc_trx_configs->bd_clk_counter_handle), connection_info_p->acl_info.acl_end_tsf)) {
    if (is_tx) {
      configure_and_schedule_tx_peri(connection_info_p, clk);
    } else {
      configure_and_schedule_rx_peri(connection_info_p, clk);
    }
  } else {
    procedure_activity_complete(connection_info_sm_p, acl_peripheral_sm_idle);
  }
}

/**
 * @brief function is used to initialise acl parameters and is used to start the acl procedure.
 * Should be seen as called HCI request
 *
 * @param hss_cmd_t *hss_cmd
 * @param btc_dev_t *btc_dev_p
 */
void acl_peripheral_start_connection(uint8_t *hss_cmd, btc_dev_t *btc_dev_p)
{
  connection_info_t *connection_info = &btc_dev_p->connection_info[*HSS_CMD_START_ACL_PERIPHERAL_CONNECTION_DEVICE_INDEX(hss_cmd)];
  connection_info->connection_info_sm = &acl_peripheral_sm;
  SM_STATE(connection_info->connection_info_sm, acl_peripheral_sm_idle);
  connection_info->connection_info_sm->current_device_index = *HSS_CMD_START_ACL_PERIPHERAL_CONNECTION_DEVICE_INDEX(hss_cmd);

  update_aperiodic_role(btc_dev_p, connection_info->connection_info_sm);

  connection_info->priority = *HSS_CMD_START_ACL_PERIPHERAL_PRIORITY(hss_cmd);
  connection_info->step_size = *HSS_CMD_START_ACL_PERIPHERAL_STEP_SIZE(hss_cmd);
  connection_info->acl_info.acl_end_tsf = *HSS_CMD_START_ACL_PERIPHERAL_END_TSF(hss_cmd);   // this end tsf will be used in all conditions for acl so that its retained.
  connection_info->acl_arqn_seqn_info.arqn_status = NAK; // Set the ARQN status as NAK
  connection_info->acl_arqn_seqn_info.seqn_status = 1; // Set the SEQN status as 1
  connection_info->acl_arqn_seqn_info.remote_seqn = 0; // Set to 0 before starting the ACL procedure
  connection_info->acl_arqn_seqn_info.remote_flow = 1; // Set to 1 before starting the ACL procedure
  btc_dev_p->current_slot_info.current_role_acl_lt_addr = connection_info->lt_addr;
  btc_dev_p->current_slot_info.current_role_lt_addr = connection_info->lt_addr;
  memcpy(connection_info->bd_address, HSS_CMD_START_ACL_PERIPHERAL_BD_ADDRESS(hss_cmd), BD_ADDR_LEN);
  memcpy(connection_info->sync_word, HSS_CMD_START_ACL_PERIPHERAL_SYNC_WORD(hss_cmd), SYNC_WORD_LEN);
  RAIL_SEQ_BTC_setAccessAddress(connection_info->sync_word);
  RAIL_SEQ_BTC_setHecInit(connection_info->bd_address[3]);
  RAIL_SEQ_BTC_setCrcInit(connection_info->bd_address[3]);

  //calculating end tsf and adding clk value that has been elapsed. (Procedure activity duration)
  connection_info->acl_info.acl_end_tsf = btc_get_current_bd_clk_cnt(&connection_info->trx_config.bd_clk_counter_handle) + (connection_info->acl_info.acl_end_tsf * HALF_SLOT_FACTOR);
  SM_STEP(ACL_PERIPHERAL_SM, connection_info->connection_info_sm, START_PROCEDURE);
}

/**
 * @brief function is used to stop acl.
 * Should be seen as called HCI request
 *
 * @param hss_cmd_t *hss_cmd
 * @param btc_dev_t *btc_dev_p
 */
void stop_acl_peripheral(uint8_t *hss_cmd, btc_dev_t *btc_dev_p)
{
  connection_info_t *connection_info = &btc_dev_p->connection_info[*HSS_CMD_STOP_ACL_PERIPHERAL_CONNECTION_DEVICE_INDEX(hss_cmd)];
  procedure_activity_complete(connection_info->connection_info_sm, acl_peripheral_sm_idle);
}

/**
 *@brief function handles transmission of ID packets for connection_info state machine.
 *@param connection_info_sm- structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 *@param bt_bd_clk_identifier- stores clock related information.
 *
 */
void acl_peripheral_start_procedure_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  btc_trx_configs_t *btc_trx_configs = &connection_info_p->trx_config;
  btc_trx_configs->btc_rail_trx_config.txPower = connection_info_p->tx_pwr_index;
  btc_trx_configs->ignore_whitening = TRUE; // TODO: Remove this once hopping is properly tested in statemachines
  btc_trx_configs->sm = connection_info_p->connection_info_sm;
  btc_trx_configs->e0EncConfig = &connection_info_p->e0EncConfig;
  btc_trx_configs->aesEncConfig = &connection_info_p->aesEncConfig;

  // Initialize multi-slot counters
  connection_info_sm_p->last_tx_slots = 0;
  connection_info_sm_p->last_rx_slots = 0;

  update_channel_info_connected_state(connection_info_sm_p);
  handle_tx_rx_peri(connection_info_p, false, BTC_BD_CLK_2NP0);
}

/**
 * @brief function is used to call functions to parse rx_packet and trigger circular buffer.
 * Should be seen as called HCI request
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 */
void acl_peripheral_header_received_done_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  arqn_stage_1_acl(connection_info_p);

  SM_STATE(connection_info_sm_p, ACL_PERIPHERAL_WAITING_FOR_RX_COMPLETE); // Updating state machine to PERIPHERAL_WAITING_FOR_RX_COMPLETE
}

void acl_peripheral_rx_poll_null_done_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  uint8_t status = arqn_stage_1_acl(connection_info_p);

  if (status) {
    arqn_stage_2_acl(connection_info_p);
  }
  handle_tx_rx_peri(connection_info_p, true, BTC_BD_CLK_2NP2);
}

/**
 * @brief function is to open for reception again,to receive packets
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 */
void acl_peripheral_header_received_timeout_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  arqn_seqn_scheme_status_t *arqn_seqn_status = &connection_info_p->acl_arqn_seqn_info;

  arqn_seqn_status->arqn_status = NAK; // Set ARQN status to NAK
  // Reset multi-slot counters since failed to detect a BTC packet
  connection_info_sm_p->last_tx_slots = 0;
  handle_tx_rx_peri(connection_info_p, false, BTC_BD_CLK_2NP0);
}

/**
 * @brief function is used to call the function to check circular buffer has been triggered or not
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 */
void acl_peripheral_rx_done_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);

  arqn_stage_2_acl(connection_info_p);
  if (TESTMODE_ENABLED == 1) {   // if testmode is enabled then loopback function is used.
    testmode_rx_done_handler(connection_info_sm_p);
    return;
  } else {
    handle_tx_rx_peri(connection_info_p, true, BTC_BD_CLK_2NP2);
  }
}

/**
 * @brief function is to open for reception again,to receive packets
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 */
void acl_peripheral_rx_timeout_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  btc_trx_configs_t *btc_trx_configs = &connection_info_p->trx_config;
  arqn_seqn_scheme_status_t *arqn_seqn_status = &connection_info_p->acl_arqn_seqn_info;

  // Reset multi-slot counters since RX failed
  connection_info_sm_p->last_rx_slots = 0;

  arqn_seqn_status->arqn_status = NAK;
  btc_trx_configs->ignore_rx_pkt = 1; // To avoid DMA transfer for CRC/HEC failed packets
  configure_and_schedule_tx_peri(connection_info_p, BTC_BD_CLK_2NP2);
}

/**
 * @brief function is to open for reception again,to receive packets
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 */
void acl_peripheral_tx_done_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  handle_tx_rx_peri(connection_info_p, false, BTC_BD_CLK_2NP0);
}

/**
 * @brief function is to open for reception again,to receive packets
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 */
void acl_peripheral_tx_abort_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  // Reset multi-slot counters since TX failed
  connection_info_sm_p->last_tx_slots = 0;
  connection_info_sm_p->last_rx_slots = 0;
  handle_tx_rx_peri(connection_info_p, false, BTC_BD_CLK_2NP0);
}

/**
 * @brief During testmodes, the received packet is supposed to be transmitted in loopback mode.
 * During this process, the received packet along with the header needs to be added to the Tx queue with proper adjustment of the packet for transmission.
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 *
 */
void loopback_testmode_add_rx_packet_to_tx_queue(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  configure_and_schedule_tx_peri(connection_info_p, BTC_BD_CLK_2NP2);
}

/**
 * @brief function is used to check for loopback mode and continuous rx modes from Testmodes.
 * In loopback mode, the received packet along with the header needs to be added to the Tx queue.
 * In continuous rx, It will go in schedule rx state.
 *
 * @param connection_info-structure of connection_info state machine, containing information of bd _address, lt address, clk_offset,clk handles,
 * current device index,flow and acl_info, esco _info
 *
 */
void testmode_rx_done_handler(sm_t *connection_info_sm_p)
{
  connection_info_t *connection_info_p = get_connection_info(connection_info_sm_p);
  if (LOOPBACKMODE == 1) {
    loopback_testmode_add_rx_packet_to_tx_queue(connection_info_sm_p);
    SM_STATE(connection_info_sm_p, ACL_PERIPHERAL_TX_SCHEDULED);       // updating state machine to PERIPHERAL_TX_SCHEDULED
  } else if (CONTINUOUS_RX == 1) {
    configure_and_schedule_rx_peri(connection_info_p, BTC_BD_CLK_2NP0);
  }
}
