...
def pytest_addoption(parser):
    parser.addoption("--fast", action="store_true", default=False, help="Skip make commands")
...
def pytest_sessionstart(session: Config):
    ...
    if not session.config.getoption("--fast"):
        subprocess.run(["make", "clean", "-j"], cwd=BUILD_PATH)
        subprocess.run(["make", "sim_test=true", "-j"], cwd=BUILD_PATH)
...
def teardown():
    ...
    subprocess.run(["bash", f"{docker_base_path}/tools/run_gcov.sh", "llc"])
